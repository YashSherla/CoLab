...
interface ProjectFile {
  name: string;
  url: string; // Assuming you'll store the URL after upload
}
  ...
  const storeFile = (file: any , index:number) => {
    return new Promise((resolve, reject)=>{
      ...
      uploadTask.on('state_changed', (snapshot) => {
        ...
        setUploadPerc(prev => ({ ...prev, [index]: Math.round(progress) }));
      }, (error) => {
        console.log(error);
        reject(error)
      }, () => {
            ...
            <div className="flex flex-col w-full space-y-5">
              ...
              {/* All Files */}
              {
               files.map((file,index:number) => {
                const percentage = uploadPerc[index] || 0;
                  return  <div key={index}>
                            <hr className="border-t border-black/[0.2] dark:border-white/[0.2] my-2 w-full" />
                            <div className="grid grid-cols-12 gap-4 items-center px-4">
                              <div className="col-span-1 flex items-center justify-center gap-1">
                                <img src={video} alt="File Type" className="w-20" />
                                <p className="text-black text-lg dark:text-white ">{file.name.split('.')[1]}</p>
                              </div>
                              <div className="col-span-7 text-center">
                                <p className="text-black text-lg text-ellipsis overflow-hidden dark:text-white px-2 whitespace-nowrap">{file.name}</p>
                              </div>
                              ...
                              <div className="col-span-2 flex items-center justify-end">
                                <FaTrash className="text-gray-500" onClick={()=>handleRemoveFile(index)} />
                              </div>
                            </div>
                          </div>
                })
